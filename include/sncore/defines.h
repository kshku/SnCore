...
#define SN_BYTE_ARRAY_SET(arr, n)      ((arr)[(n) / 8] |= (uint8_t)(1 << ((n) % 8)))
#define SN_BYTE_ARRAY_CLEAR(arr, n)    ((arr)[(n) / 8] &= (uint8_t)~(1 << ((n) % 8)))
#define SN_BYTE_ARRAY_TOGGLE(arr, n)   ((arr)[(n) / 8] ^= (uint8_t)(1 << ((n) % 8)))
#define SN_BYTE_ARRAY_CHECK(arr, n)    ((arr)[(n) / 8] & (uint8_t)(1 << ((n) % 8)))
